...
import com.figo.gpslogger.loggers.FileLoggerFactory;
import com.figo.gpslogger.loggers.IFileLogger;
import com.figo.gpslogger.senders.AlarmReceiver;
...
public class GpsLoggingService extends Service implements IActionListener {
	...
	public void onDestroy() {
		Utilities
				.LogWarning("GpsLoggingService is being destroyed by Android OS.");
	...
	private void HandleIntent(Intent intent) {
		...
		if (intent != null) {
			...
			if (bundle != null) {
				...
				Utilities.LogDebug("startRightNow - "
						+ String.valueOf(startRightNow));

				Utilities.LogDebug("emailAlarm - "
						+ String.valueOf(sendEmailNow));
			...
			// restarted.
			Utilities
					.LogDebug("Service restarted with null intent. Start logging.");
			StartLogging();
	...
	public void SetupAutoSendTimers() {
		...
		Utilities.LogDebug("isAutoSendEnabled - "
				+ String.valueOf(AppSettings.isAutoSendEnabled()));
		Utilities.LogDebug("Session.getAutoSendDelay - "
	...
	private void CancelAlarm() {
		...
		if (alarmIntent != null) {
			...
			PendingIntent sender = PendingIntent.getBroadcast(this, 0,
					alarmIntent, PendingIntent.FLAG_UPDATE_CURRENT);
			Utilities.LogDebug("Pending alarm intent was null? "
	 ...
	 * Utilities.LogVerbose("isAutoSendEnabled - " +
	 * AppSettings.isAutoSendEnabled()); // autoSendDelay 0 means send it when
	 * you stop logging. if (AppSettings.isAutoSendEnabled() &&
	 * Session.getAutoSendDelay() == 0) { Session.setReadyToBeAutoSent(true);
	 * AutoSendLogFile(); } }
	 */
	 ...
	 * Utilities.LogVerbose("isReadyToBeAutoSent - " +
	 * Session.isReadyToBeAutoSent());
	 * 
	 * // Check that auto emailing is enabled, there's a valid location and //
	 * file name. if (Session.getCurrentFileName() != null &&
	 ...
	 * Session.setReadyToBeAutoSent(true); SetupAutoSendTimers();
	 * 
	 * } }
	 * 
	 * protected void ForceEmailLogFile() {
	 * 
	 * Utilities.LogDebug("GpsLoggingService.ForceEmailLogFile"); if
	 * (AppSettings.isAutoSendEnabled() && Session.getCurrentFileName() != null
	 * && Session.getCurrentFileName().length() > 0) { if (IsMainFormVisible())
	 * { Utilities.ShowProgress(mainServiceClient.GetActivity(),
	 * getString(R.string.autosend_sending), getString(R.string.please_wait)); }
	 * 
	...
	private void GetPreferences() {
		...
		Utilities.LogDebug("Session.getAutoSendDelay: "
				+ Session.getAutoSendDelay());
		Utilities.LogDebug("AppSettings.getAutoSendDelay: "
				+ AppSettings.getAutoSendDelay());

		if (Session.getAutoSendDelay() != AppSettings.getAutoSendDelay()) {
			Utilities.LogDebug("Old autoSendDelay - "
					+ String.valueOf(Session.getAutoSendDelay()) + "; New -"
	...
	private void ShowNotification() {
		...
		Notification nfc = new Notification(R.drawable.gpsloggericon2, null,
		...
		nfc.setLatestEventInfo(getApplicationContext(),
				getString(R.string.gpslogger_still_running), contentText,
				pending);
	...
	private void CheckTowerAndGpsStatus() {
		Session.setTowerEnabled(towerLocationManager
				.isProviderEnabled(LocationManager.NETWORK_PROVIDER));
		Session.setGpsEnabled(gpsLocationManager
	...
	void OnLocationChanged(Location loc) {
		...
		if (!Session.isStarted()) {
			Utilities
					.LogDebug("OnLocationChanged called, but Session.isStarted is false");
			StopLogging();
			return;
		}
		...
		if ((currentTimeStamp - Session.getLatestTimeStamp()) < (AppSettings
				.getMinimumSeconds() * 1000)) {
			return;
		}
	...
	private void SetDistanceTraveled(Location loc) {
		...
		// Calculate this location and the previous location location and add to
		// the current running total distance.
		// NOTE: Should be used in conjunction with 'distance required before
		// logging' for more realistic values.
		double distance = Utilities.CalculateDistance(
				Session.getPreviousLatitude(), Session.getPreviousLongitude(),
	...
	protected void StopManagerAndResetAlarm(int retryInterval) {
		Utilities
				.LogDebug("GpsLoggingService.StopManagerAndResetAlarm_retryInterval");
		StopGpsManager();
	...
	private void SetAlarmForNextPoint() {
		...
		nextPointAlarmManager.set(AlarmManager.ELAPSED_REALTIME_WAKEUP,
				SystemClock.elapsedRealtime() + AppSettings.getMinimumSeconds()
						* 1000, pi);
	...
	private void SetAlarmForNextPoint(int retryInterval) {
		...
		Utilities
				.LogDebug("GpsLoggingService.SetAlarmForNextPoint_retryInterval");
